...
const jdok   = '{"Jd OK":0}';
const jdlogx = '{"Jd error":"logon required"}';
const jqwait = '{"Jd error":"timeout - wait for port"}';
const jdwait = '{"Jd error":"timeout - wait for Jd"}';
...
const RDOPS= ['close','read','reads','key','info','list']; // 'rd...','xrd...'
...
setInterval(() => {timeouts()},inttimeout);
...
function doit(req,res){
  if(req.method == 'POST')
  {
    dopost(req, res, function() {
      ...
      if(i==-1){logit(res,'on-');reply(200,res,jdlogx);return;}
      ...
      if(0==res.jdop.length){logit(res,'mt');reply(200,res,JDOK);return}
...
async function run(res) {
  ...
  try{ 
    ...
    if(res.jdcookiereq!=res.jdcookie){
      res.writeHead(200, "OK", {'Set-Cookie':"jds_cookie="+res.jdcookie+";path=/;Secure;Httponly",'Content-Type': 'text/html'});
      res.end(p);
    }else{
...
// timeout reply to those on jobq too long or in jd too long
function timeouts(){
  ...
  for (let i = 0; i < jobq.length; i++){
    ...
    if(jqtimeout<(ts-res.jdts)){
      // remove from q and provide respons
      jobq.splice(i,1);
      res.writeHead(200, "OK", {'Content-Type': 'application/octet-stream'});
      res.end(jqwait);
    }
